 ...
 * @retval 无
 */
void ILI9341_DrawBmp(uint16_t usX, //字符显示位置x
												uint16_t usY, //字符显示位置y
												uint8_t width,
												uint8_t height,
												uint8_t p)						//字模数据
{
  uint32_t index = 0, counter = 0, bitCount=0;
	...
	//按字节读取字模数据
	//由于前面直接设置了显示窗口，显示数据会自动换行
	for ( index = 0; index < width*height/128; index++ )
	{
			for ( counter = 0; counter < 16; counter++ )
			{
				...
				for ( bitCount = 0; bitCount < 8; bitCount++ )
				{
						...
						if ( icon32[32*p+index][counter] & (0x80>>bitCount) )
							ILI9341_Write_Data ( CurrentTextColor );		
						else
							ILI9341_Write_Data ( CurrentBackColor );	
...
void switch_system_back(void) {
	if(system_back == BLACK) system_back = WHITE; else system_back = BLACK;
	first_display = 1;
}
